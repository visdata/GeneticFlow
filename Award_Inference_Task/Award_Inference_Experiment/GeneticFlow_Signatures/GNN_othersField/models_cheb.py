...
import numpy as np
from torch_geometric.nn import global_mean_pool as gap, global_max_pool as gmp, GlobalAttention, GraphMultisetTransformer, Set2Set
...
class Model(torch.nn.Module):
    def __init__(self, args):
        ...
        self.lamb = args.lamb
        self.edgeconv = MLP(10, 64, 1).to(device)
        ...
        # self.fieldID_embedding=args.fieldID_embedding
        # if self.fieldID_embedding>0:
        #     self.embedding = nn.Embedding(args.num_field, self.fieldID_embedding)
        # GCN2
        # alpha=0.1
        # theta=0.5
        ...
        if self.conv_name == 'ChebConv':
            ...
            nn1 = Sequential(
                Linear(self.num_features, hidden_channels),
    ...
    def forward(self, data, paper_count):
        ...
        # if self.fieldID_embedding>0:
        #     fieldID= data.field_id.long().reshape(-1).to(device)
        #     fieldID_embedding=self.embedding(fieldID)
        #     x=torch.cat((x,fieldID_embedding),dim=1)
...
class MEWISPool(nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        if edge_index.size(1) != 0:
            ...
            A = torch.zeros([batch_nodes, batch_nodes]).to(device)
    ...
    def graph_reconstruction(mewis, x, A):
        ...
        I = torch.eye(len(mewis)).to(device)
        one = torch.ones([len(mewis), len(mewis)]).to(device)
...
class Net3(nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        x = self.gc1(x, edge_index).to(device)
        x = torch.relu(x)
