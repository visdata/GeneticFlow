...
for root, dirs, files in os.walk(".", topdown=False):
    for name in files:
        if(name[0]=='p'):
            ...
            vertex[0]=vertex[0].astype('string')

            # try:
            #     arc=pd.read_csv("links"+name[6:], header = None)
            #     arc=arc.iloc[:,1].values.reshape(-1)
            #     arc=arc.astype('str')
            #     print(vertex)
            #     vertex = vertex[vertex[0].isin(arc)]
            #     print(vertex)
            # except:
            #     continue

            # vertex=vertex[vertex[6].astype(float) >= 0.5]
            vertex=vertex.iloc[:,[0,2,3,4,5,6,7,10,11]]
            ...
            np.savetxt("../data_origin/"+name[:-4]+"_new.csv", np.array(vertex), fmt="%s", delimiter="	")
            ...
            # print(len(arc_new))
            # # sleep()
            # np.savetxt("../data_origin/"+name[:-4]+"_new.csv", np.array(arc_new), fmt="%s", delimiter="	")