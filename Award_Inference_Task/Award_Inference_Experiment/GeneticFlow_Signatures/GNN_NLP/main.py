...
if __name__ == '__main__':
    ...
    if end_path == 'data':
        ...
        recordfile+=end_path
    recordfile+='.txt'
    comment=f"--PATH {args.PATH} --lr {args.lr} --nhid {args.nhid} --dropout_ratio {args.dropout_ratio} --conv_name {args.conv_name} --num_layers {args.num_layers} --average({args.average}) {comment} "
    print_to_file(recordfile,comment+f"{round}*10-fold-cv Precision:%.3f Recall:%.3f F1_best:%.3f F1_best(std): %.3f ROC:%.3f ROC(std): %.3f PRC:%.3f ACC:%.3f for Class Fellow" % (np.mean(Precision),np.mean(Recall),np.mean(F1_best),np.std(F1_best),np.mean(ROC),np.std(ROC),np.mean(PRC),np.mean(ACC)))
