    ...
    def __init__(self, args):
        ...
        for layer in range(self.num_layers - 1):
            ...
            if self.pool_name == 'SAGPooling':
                self.pool.append(SAGPooling(self.nhid, ratio=self.pooling_ratio, GNN=GCNConv))
            elif self.pool_name == 'TopKPooling':
                self.pool.append(TopKPooling(self.nhid, ratio=self.pooling_ratio))
            elif self.pool_name == 'ASAPooling':
                self.pool.append(ASAPooling(self.nhid, ratio=self.pooling_ratio, dropout=0.5, GNN=GCNConv, add_self_loops=False))
            elif self.pool_name == 'EdgePooling':
                self.pool.append(EdgePooling(self.nhid, edge_score_method=EdgePooling.compute_edge_score_softmax, dropout=self.pooling_ratio, add_to_edge_score=1))
            elif self.pool_name == 'HGPSLPool':
                self.pool.append(HGPSLPool(self.nhid, self.pooling_ratio, self.sample, self.sparse, self.sl, self.lamb))
    ...
    def forward(self, data, paper_count,flag):
        ...
        for layer in range(self.num_layers):
            ...
            if layer == self.num_layers - 1:
                ...
                if self.pool_name == 'SAGPooling':
                    h, edge_index, edge_attr, batch, _, _ = self.pool[layer](h, edge_index, edge_attr, batch)
                elif self.pool_name == 'TopKPooling':
                    h, edge_index, edge_attr, batch, _, _ = self.pool[layer](h, edge_index, edge_attr, batch)
                elif self.pool_name == 'ASAPooling':
                    h, edge_index, edge_attr, batch, _ = self.pool[layer](h, edge_index, edge_attr, batch)
                elif self.pool_name == 'EdgePooling':
                    h, edge_index, batch, _ = self.pool[layer](h, edge_index, batch)
                elif self.pool_name == 'HGPSLPool':
                    h, edge_index, edge_attr, batch = self.pool[layer](h, edge_index, edge_attr, batch)
            Batch.append(batch)
            # if self.residual:
            #     h += h_list[layer]
            h_list.append(h)
        ...
        for layer in range(self.num_layers):
            if(self.average):
                node_representation += F.relu(gap(h_list[layer + 1], Batch[layer]))
            else:
                node_representation += F.relu(gmp(h_list[layer + 1], Batch[layer]))
